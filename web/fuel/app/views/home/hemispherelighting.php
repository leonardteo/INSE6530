<?php
/**
 * Test of Hemisphere Lighting
 */
?>

<h1>Hemispherical Lighting</h1>

<p>Description: Test of Hemispherical Lighting Shader. The models are shown without textures to show the effect.</p>
<p>Usage: </p>
<ul>
...
<script type="text/javascript">
	...
	function main(){
		...
		var shader = new Shader("/assets/shaders/hemispherelighting.vs", "/assets/shaders/hemispherelighting.fs");
		
		//Check if the shader is compiled
		if (!shader.programReady){
			updateStatus("Shader compile failed");
			return;
		}
		...
		//Create a plane
		var plane = new MeshNode("plane");
		plane.loadPlane(50);
		plane.loadShader("/assets/shaders/groundplane.vs", "/assets/shaders/groundplane.fs");
		
		//Check if the shader is compiled
		if (!plane.shader.programReady){
			updateStatus("Shader compile failed");
			return;
		}		
		...
		sceneGraph.rootNode.addChild(plane);
